...
select ograd, count(*) as isimSayisi from ogrenci group by ograd;

-- Task 2: Sınıflardaki öğrenci sayısını bulun
select sinif, count(*) as sinif_ogr_sayisi from ogrenci group by sinif;
...
select turno, count(idno) as kitap_sayisi from kitap group by turno; 

-- Task 5: Her türdeki en fazla sayfa sayısı olan kitapları listeleyiniz.
select turno, max(sayfasayisi) as enFazla_sayfali_Kitap from kitap group by turno; 
-- Task 6: Her türdeki en az sayfa sayısı olan kitapları listeleyiniz.
select turno, min(sayfasayisi) as enaz_sayfali_Kitap from kitap group by turno;
-- Task 7***: Her türden kaç tane kitap olduğunu listeleyiniz.(Tür isimleri de olsun)
select kitap_turu, (select count(turno) from kitap where islemler.turno=kitap.turno) as adet_sayisi from islemler;
